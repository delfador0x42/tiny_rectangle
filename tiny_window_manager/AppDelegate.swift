...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func checkForConflictingApps() {
        ...
        for app in runningApps {
            ...
            if let conflictingAppName = conflictingAppsIds[bundleId] {
                AlertUtil.oneButtonAlert(question: "Potential window manager conflict: \(conflictingAppName)", text: "Since \(conflictingAppName) might have some overlapping behavior with tiny_window_manager, it's recommended that you either disable or quit \(conflictingAppName).")
                break
            }
        }
    ...
    func checkForProblematicApps() {
        ...
        if !problemBundles.isEmpty {
            AlertUtil.oneButtonAlert(question: "Known issues with installed applications", text: "\(displayNameString)\n\nThese applications have issues with the drag to screen edge to snap functionality in tiny_window_manager.\n\nYou can either ignore the applications using the menu item in tiny_window_manager, or disable drag to screen edge snapping in tiny_window_manager preferences.")
