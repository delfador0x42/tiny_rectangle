...
extension Defaults {
    static func encoded() -> String? {
        ...
        let config = Config(bundleId: "com.knollsoft.tiny_window_manager",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let tiny_window_managerSupportURL = getSupportDir()?
            .appendingPathComponent("tiny_window_manager", isDirectory: true) {
            
            let configURL = tiny_window_managerSupportURL.appendingPathComponent("tiny_window_managerConfig.json")
            ...
            if exists == true {
                ...
                do {
                    ...
                    try FileManager.default.moveItem(atPath: configURL.path, toPath: tiny_window_managerSupportURL.appendingPathComponent(newFilename).path)
                } catch {
                    do {
                        try FileManager.default.removeItem(at: configURL)
                    } catch {
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove tiny_window_managerConfig.json from \(tiny_window_managerSupportURL) after loading.")
                    }
                }
            }
        }
