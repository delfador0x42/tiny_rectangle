...
//                                        │ BL  │ BR  │  (bottom row)
//                                        └─────┴─────┘
// [TL] = This class positions the window here (top-left)

/// Positions a window in the top-left sixth of the screen.
///
/// In landscape: The left cell of the top row (1/3 width, 1/2 height).
/// In portrait: The left cell of the top row (1/2 width, 1/3 height).
///
/// When repeatedly triggered, cycles rightward through the top row:
/// top-left → top-center → top-right → (repeat)
class TopLeftSixthCalculation: WindowCalculation, OrientationAware, SixthsRepeated {
    ...
    private func getNextCalculationInCycle(_ params: RectCalculationParameters) -> SimpleCalc? {
        ...
        // Only cycle if we're continuing from a top-left-sixth action
        let isRelevantAction = lastAction.action == .topLeftSixth
        let isRelevantSubAction = lastSubAction == .topLeftSixthLandscape
        ...
        // Cycle rightward through the top row: top-left → top-center → top-right
